...
class PredClick extends StatefulWidget {
  const PredClick({super.key, this.pickedImage});
  final File? pickedImage;

  @override
  State<PredClick> createState() => _PredClickState();
}

class _PredClickState extends State<PredClick> {
  File? _selectedImg;
  ...
  Widget build(context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Currency Click"),
        actions: [
          IconButton(
              onPressed: () {
                setState(() {
  ...
  void _pickPicture() async {
    XFile? pickedImg =
        await ip.pickImage(source: ImageSource.camera, maxHeight: 600);
